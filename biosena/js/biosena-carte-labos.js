...
function filtering() {
  for (var labo in data) {
    if (active.length === 0) { // if no button is clicked and therefore the Active array is empty
      ...
      data[labo].visible = 0;  // first change all visilibities to 0
      if (active.every(r=> data[labo].domaine.includes(r))) { // if the any element of the Domain array matches with any element of the Active array //// CHANGE SOME TO EVERY TO GO FROM INCLUSIVE TO EXCLUSIVE, OR vs AND
